...
class ScanNetDataset(DefaultDataset):
    ...
    def get_data(self, idx):
        ...
        if "segment20" in data_dict.keys():
            ...
            if split == 'train' and self.weak_label is not None:
                # r = 0.1
                # r = 1
                r = self.weak_label
                r = float(r)/100
                ...
                M = max(2, int(segment.shape[0] * r))
                # M = 20
                # print("M===>>",M)
                # M = 20
                valid_ind = np.where(segment != self.ignore_index)[0]
                random_ind = np.random.choice(valid_ind , M, replace=False)

                samples_label = -1 * np.ones(segment.shape[0], dtype=np.int32)
                samples_label[random_ind] = segment[random_ind]

                segment = samples_label
