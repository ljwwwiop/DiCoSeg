...
class SemanticKITTIDataset(DefaultDataset):
    def __init__(self, ignore_index=-1, **kwargs):
        ...
        # self.weak_label = kwargs.get('weak_label', None)
    ...
    def get_data(self, idx):
        ...
        if os.path.exists(label_file):
            ...
            if self.weak_label is not None:
                ### here is weakly supervised
                # r = 0.01 # 1%
                # r = 0.001 # 0.1%
                r = float(self.weak_label / 100)
                ...
                # 创建一个新的标签数组，初始化为 -1
                sampled_labels = -1 * np.ones(segment.shape[0], dtype=np.int64)
                # 将随机选择的 M 个标签填充到新的数组中
                sampled_labels[random_indices] = segment[random_indices]

                segment = sampled_labels        
        else:
