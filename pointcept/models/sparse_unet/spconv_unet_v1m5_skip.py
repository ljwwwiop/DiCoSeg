...
class ConsistentSPConvDown(nn.Module):
    ...
    def forward(self, x):
        ...
        # 加权特征增强
        out_saliency = scatter(saliency[in_indices], out_indices, 
                             dim_size=out.features.shape[0], reduce="max")
        enhanced_feats = out.features * (1 + out_saliency)
...
@MODELS.register_module("SpUNet-v1m5-skip")
class SpUNetBase(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        for s in range(self.num_stages):
            ...
            # before layer feature as resdual
            # ---- 跨层特征融合 ----
            if s > 0:  # 从第二层开始融合
                # 1. 坐标对齐（处理下采样导致的点集变化）
