...
class PointTransformerSegSkip(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            seg_masks = (segment != -1)
            print("origin size==>>",seg_masks.sum())

        # encoder
        p1, x1, o1 = self.enc1([p0, x0, o0]) # stem

        # 建立坐标到索引映射字典（key 是坐标元组）
        p1_np = p1.cpu().numpy()
        coord_to_index = {tuple(coord): idx for idx, coord in enumerate(p1_np)}


        p2, x2, o2 = self.enc2([p1, x1, o1])
        if self.training:
            # 转为 CPU numpy（避免 torch 字典慢）
            p2_np = p2.cpu().numpy()
            segment_np = segment.cpu().numpy()

            # 查找 p2 中每个坐标在 p1 中的索引
            p2_index_in_p1 = [coord_to_index[tuple(coord)] for coord in p2_np]

            # 映射标签
            segment_p2 = torch.from_numpy(segment_np[p2_index_in_p1]).to(p1.device)
            print("segment_p2 size==>>",(segment_p2 > -1).sum(),"/",p2.shape[0])
        ...
        if self.training:
            ...
            # 查找 p2 中每个坐标在 p1 中的索引
            p3_index_in_p1 = [coord_to_index[tuple(coord)] for coord in p3_np]

            # 映射标签
            segment_p3 = torch.from_numpy(segment_np[p3_index_in_p1]).to(p1.device)
            print("segment_p3 size==>>",(segment_p3 > -1).sum(),"/",p3.shape[0])
        ...
        if self.training:
            ...
            # 查找 p2 中每个坐标在 p1 中的索引
            p4_index_in_p1 = [coord_to_index[tuple(coord)] for coord in p4_np]

            # 映射标签
            segment_p4 = torch.from_numpy(segment_np[p4_index_in_p1]).to(p1.device)
            print("segment_p4 size==>>",(segment_p4 > -1).sum(),"/",p4.shape[0])
        ...
        if self.training:
            ...
            # 查找 p2 中每个坐标在 p1 中的索引
            p5_index_in_p1 = [coord_to_index[tuple(coord)] for coord in p5_np]

            # 映射标签
            segment_p5 = torch.from_numpy(segment_np[p5_index_in_p1]).to(p1.device)
            print("segment_p5 size==>>",(segment_p5 > -1).sum(),"/",p5.shape[0])
...
@MODELS.register_module("PointTransformerSkip-Seg26")
class PointTransformerSegSkip26(PointTransformerSegSkip):
    def __init__(self, **kwargs):
        super(PointTransformerSegSkip26, self).__init__(
            Bottleneck, [1, 1, 1, 1, 1], **kwargs
        )


@MODELS.register_module("PointTransformerSkip-Seg38")
class PointTransformerSegSkip38(PointTransformerSegSkip):
    def __init__(self, **kwargs):
        super(PointTransformerSegSkip38, self).__init__(
            Bottleneck, [1, 2, 2, 2, 2], **kwargs
        )


@MODELS.register_module("PointTransformerSkip-Seg50")
class PointTransformerSegSkip50(PointTransformerSegSkip):
    def __init__(self, **kwargs):
        super(PointTransformerSegSkip50, self).__init__(
